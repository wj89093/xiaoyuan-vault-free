...
function onEvent(channel: string, callback: (...args: unknown[]) => void): () => void {
  const sub = (...args: unknown[]) => {
    ...
    try {
      ...
      console.error('[preload] handler crash on', channel, 'args:', JSON.stringify(args).slice(0, 200), e)
    }
  }
...
const file = {
  ...
  create: (path: string, title: string, type?: string) => handler<boolean>('file:create', path, title, type),
  ...
  delete: (vaultPath: string, filePath: string) => handler<boolean>('file:delete', vaultPath, filePath),
  move: (filePath: string, newParentDir: string) => handler<boolean>('file:move', filePath, newParentDir),
  revealInFinder: (filePath: string) => handler<void>('file:revealInFinder', filePath),
  openExternal: (filePath: string) => handler<{ ok: boolean; error?: string }>('file:openExternal', filePath),
  trashList: (vaultPath: string) => handler<any[]>('file:trashList', vaultPath),
  trashRestore: (vaultPath: string, originalPath: string) => handler<boolean>('file:trashRestore', vaultPath, originalPath),
  trashDelete: (vaultPath: string, originalPath: string) => handler<boolean>('file:trashDelete', vaultPath, originalPath),
  trashClean: (vaultPath: string) => handler<number>('file:trashClean', vaultPath),
  import: (vaultPath: string, filePaths: string[]) => handler<ImportFileResult[]>('file:import', vaultPath, filePaths),
  listRaw: (vaultPath: string) => handler<any[]>('file:listRaw', vaultPath),

  convertRaw: (rawPath: string, vaultPath: string) => handler<any>('file:convertRaw', rawPath, vaultPath),
  ...
  previewBackup: (filePath: string, timestamp: string) => handler<string>('file:previewBackup', filePath, timestamp),
  restoreBackup: (filePath: string, timestamp: string) => handler<boolean>('file:restoreBackup', filePath, timestamp),
...
const url = {
  ...
  saveUrl: (vaultPath: string, title: string, content: string) => handler<string>('import:saveUrl', vaultPath, title, content),
}
...
const chat = {
  ask: (question: string, history?: ChatMessage[]) => handler<AskResult>('chat:ask', question, history ?? []),
  ...
  onStreamChunk: (cb: (data: { chunk: string }) => void) =>
    onEvent('chat:streamChunk', cb as any),
  onGreeting: (cb: (data: { text: string }) => void) =>
    onEvent('chat:greeting', cb as any),
  onStreamDone: (cb: (data: AskResult) => void) =>
    onEvent('chat:streamDone', cb as any),
  onStreamError: (cb: (data: { error: string }) => void) =>
    onEvent('chat:streamError', cb as any),
  onAborted: (cb: () => void) =>
    onEvent('chat:aborted', cb as any),
  onToolUpdate: (cb: (data: { name: string; args: unknown; status: string; result?: string }) => void) =>
...
const auth = {
  ...
  debugLogin: (email: string, code: string) => handler<string, string>('auth:debugLogin', email, code),
...
const settings = {
  ...
  getAgentPlugin: () => handler<{ enabled: boolean; endpoint: string; apiKey: string; protocol: 'ws' | 'http'; name: string }>('settings:getAgentPlugin'),
  setAgentPlugin: (config: { enabled: boolean; endpoint: string; apiKey: string; protocol: 'ws' | 'http'; name: string }) => handler<any>('settings:setAgentPlugin', config),
  setAgentPluginEnabled: (enabled: boolean) => handler<boolean>('settings:setAgentPluginEnabled', enabled),
}
...
const maintenance = {
  ...
  saveConversation: (params: { title: string; topic: string; decisions: string[]; relatedFiles: string[]; nextSteps: string[]; discussion?: string }) =>
...
const shortcuts = {
  onImportCompleted: (cb: (filePaths?: string[]) => void) =>
    onEvent('import:completed', cb as any),
  onQuickSwitch: (cb: () => void) =>
    onEvent('shortcut:quick-switch', cb as any),
  onGotoImport: (cb: () => void) =>
    onEvent('shortcut:goto-import', cb as any),
}
...
const skill = {
  ...
  listTemplates: () => handler<SkillTemplate[]>('skill:listTemplates'),
  loadTemplate: (name: string) => handler<string>('skill:loadTemplate', name),
  getEndpoint: () => handler<EndpointConfig>('skill:getEndpoint'),
  setEndpoint: (config: Partial<EndpointConfig>) => handler<boolean>('skill:setEndpoint', config),
}
...
const _agent = {
  onBashChunk: (cb: (data: { chunk: string }) => void) =>
    onEvent('bash:chunk', cb as any),
  onBashDone: (cb: (data: { result: string }) => void) =>
    onEvent('bash:done', cb as any),
...
const api = {
  ...
  trashRestore: (vaultPath: string, originalPath: string) => file.trashRestore(vaultPath, originalPath),
  trashDelete: (vaultPath: string, originalPath: string) => file.trashDelete(vaultPath, originalPath),
  ...
  selectDirectory: () => handler<string | null>('dialog:selectDirectory'),
  getBuildInfo: () => handler<{ isPro: boolean; isOpenSource: boolean; buildTarget: string }>('app:buildInfo'),
  ...
  skillListTemplates: () => skill.listTemplates(),
  skillLoadTemplate: (name: string) => skill.loadTemplate(name),
  skillGetEndpoint: () => skill.getEndpoint(),
  skillSetEndpoint: (config: Partial<EndpointConfig>) => skill.setEndpoint(config),
}
