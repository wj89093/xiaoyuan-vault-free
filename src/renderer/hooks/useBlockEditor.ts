...
function _startEdit(widget: BlockWidget, dom: HTMLElement, view: EditorView): void {
  ...
  Object.assign(ta.style, {
    width: '100%', minHeight: '100px', display: 'block', boxSizing: 'border-box',
    fontFamily: 'var(--font-mono,monospace)', fontSize: '13px', padding: '12px',
    border: '2px solid var(--color-primary,#1a56a8)', borderRadius: '6px',
    background: 'var(--color-bg-primary,#fff)', color: 'var(--color-text-primary,#1d1d1f)',
  ...
  Object.assign(btns.style, { display: 'flex', gap: '8px', marginTop: '8px', justifyContent: 'flex-end' })

  const save = document.createElement('button')
  save.textContent = '保存'
  Object.assign(save.style, {
    padding: '4px 16px', borderRadius: '6px', border: 'none',
    background: 'var(--color-primary,#1a56a8)', color: '#fff', cursor: 'pointer', fontSize: '13px',
  })
  ...
  Object.assign(cancel.style, {
    ...
    background: 'var(--color-surface,#f5f5f7)', color: 'var(--color-text-secondary,#6e6e73)',
  ...
  ta.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { e.stopPropagation(); abortEdit() }
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { e.stopPropagation(); confirmEdit(from, to) }
  })

  save.addEventListener('click', (e) => { e.stopPropagation(); confirmEdit(from, to) })
  cancel.addEventListener('click', (e) => { e.stopPropagation(); abortEdit() })
...
function abortEdit(): void {
  ...
  const docLen = view.state.doc.length
  const wfrom = (widget as any).from, wto = (widget as any).to
