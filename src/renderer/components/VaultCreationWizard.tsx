...
function StepIndicator({ current }: { current: number }) {
  return (
    <div className="vcw-step-indicator" role="progressbar" aria-valuenow={current} aria-valuemin={1} aria-valuemax={TOTAL_STEPS}>
      {Array.from({ length: TOTAL_STEPS }, (_, i) => {
        ...
        return (
          <React.Fragment key={step}>
            <div className={`vcw-step-dot ${active ? 'vcw-step-active' : ''} ${done ? 'vcw-step-done' : ''}`}
              aria-label={`步骤 ${step} / ${TOTAL_STEPS}`}>
              {done ? <Check size={10} strokeWidth={3} /> : step}
            </div>
            {step < TOTAL_STEPS && <div className={`vcw-step-line ${done ? 'vcw-step-line-done' : ''}`} />}
          </React.Fragment>
        )
      })}
    </div>
...
export function VaultCreationWizard({ onClose, onCreated }: VaultCreationWizardProps): JSX.Element {
  ...
  const handleNext = () => { setStepDirection('forward'); _setPrevStep(step); setStep(s => s + 1) }
  const handleBack = () => { setStepDirection('back'); _setPrevStep(step); setStep(s => s - 1) }
  ...
  return (
    <div
      ...
      <div style={{
        background: 'var(--color-surface)', borderRadius: 12, padding: 32, width: 440,
        boxShadow: '0 8px 40px rgba(0,0,0,0.18)',
        display: 'flex', flexDirection: 'column', gap: 20,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 16, fontWeight: 600 }}>新建知识库</span>
          <button onClick={onClose} tabIndex={0} aria-label="关闭" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-tertiary)' }}>
            <X size={18} />
          </button>
        </div>
        ...
        {step === 1 && (
          <>
            <div>
              <label style={{ fontSize: 12, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 6 }}>名称</label>
              <input
                value={name}
                onChange={e => setName(e.target.value)}
                style={{
                  width: '100%', padding: '10px 12px', borderRadius: 8,
                  border: nameError ? '1px solid var(--color-red)' : '1px solid var(--color-border)',
              ...
              {nameError && (
                <div id="vcw-name-error" style={{ fontSize: 11, color: 'var(--color-red)', marginTop: 4 }} role="alert">
                  {nameError}
                </div>
              )}
            </div>
            <div>
              <label style={{ fontSize: 12, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 6 }}>位置</label>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  value={basePath}
                  readOnly
                  style={{
                    flex: 1, padding: '10px 12px', borderRadius: 8, border: '1px solid var(--color-border)',
                  ...
                  placeholder="选择文件夹..."
                />
                <button onClick={handleBrowse} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '10px 14px', borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)', cursor: 'pointer', fontSize: 13, color: 'var(--color-text-primary)' }}>
        ...
        {step === 3 && (
          <div style={{ background: 'var(--color-surface-hover)', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 8 }}>确认知识库信息</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div><span style={{ fontSize: 11, color: 'var(--color-text-tertiary)' }}>名称：</span><strong>{name}</strong></div>
              <div><span style={{ fontSize: 11, color: 'var(--color-text-tertiary)' }}>位置：</span><span style={{ fontSize: 12, wordBreak: 'break-all' }}>{basePath}</span></div>
              {description && <div><span style={{ fontSize: 11, color: 'var(--color-text-tertiary)' }}>描述：</span><span>{description}</span></div>}
            </div>
          </div>
        )}

        </div>{/* /P3-5 step content */}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          {step > 1 && (
            <button
              onClick={handleBack}
              style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)', cursor: 'pointer', fontSize: 13, color: 'var(--color-text-primary)' }}
            >
              上一步
            </button>
          )}
          {step < TOTAL_STEPS ? (
            ...
            <button
              ...
              style={{
                ...
                background: (step === 1 ? !isStep1Valid || !!nameError : !isStep2Valid) ? 'var(--color-border)' : 'var(--color-blue)',
                color: 'var(--color-text-inverse)', fontSize: 13, fontWeight: 500,
                cursor: (step === 1 ? !isStep1Valid || !!nameError : !isStep2Valid) ? 'default' : 'pointer',
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 20px', borderRadius: 8, border: 'none',
                background: (!isStep1Valid || !!nameError || creating) ? 'var(--color-border)' : 'var(--color-green, #1e7a4d)',
                color: 'var(--color-text-inverse)', fontSize: 13, fontWeight: 500,
                cursor: (!isStep1Valid || !!nameError || creating) ? 'default' : 'pointer',
              }}
            >
              {creating ? '创建中...' : <>创建知识库 <ArrowRight size={14} /></>}
            </button>
          )}
        </div>
      </div>
