...
function renderLogMd(raw: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (h3) {
      if (inList) { out.push('</ul>'); inList = false }
      out.push(`<h3 style="font-size:12px;font-weight:700;margin:10px 0 4px;color:var(--color-text-primary)">${processLine(h3[1])}</h3>`)
      continue
    }
    const h2 = line.match(/^## (.+)/)
    if (h2) {
      if (inList) { out.push('</ul>'); inList = false }
      out.push(`<h2 style="font-size:13px;font-weight:700;margin:12px 0 4px;color:var(--color-accent,#3db872);border-bottom:1px solid var(--color-border);padding-bottom:2px">${processLine(h2[1])}</h2>`)
      continue
    }
    const h1 = line.match(/^# (.+)/)
    if (h1) {
      if (inList) { out.push('</ul>'); inList = false }
      out.push(`<h1 style="font-size:15px;font-weight:700;margin:16px 0 6px;color:var(--color-text-primary)">${processLine(h1[1])}</h1>`)
      continue
    }
    ...
    if (li) {
      if (!inList) { out.push('<ul style="margin:2px 0;padding-left:16px">'); inList = true }
...
export function LogPanel({ onClose, onSelectFile }: LogPanelProps): JSX.Element {
  ...
  const loadLog = async (isInitial = false) => {
    ...
    try {
      ...
      if (!vaultPath) { setContent(''); if (isInitial) setLoading(false); return }
      ...
      const now = new Date()
      setLastRefresh(now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' }))
    } catch {
      setLoadError(true)
    }
  ...
  // Initial load — suppress set-state-in-effect: loadLog reads vault once on mount, not a subscription
  useEffect(() => { void loadLog(true) }, [])  // eslint-disable-line react-hooks/set-state-in-effect

  // Auto-poll every 2s + bash:chunk triggers refresh + import:completed
  useEffect(() => {
    pollTimerRef.current = setInterval(() => { void loadLog() }, 2000)

    const unsubChunk = (window.api as any).agent?.onBashChunk?.(() => { void loadLog() })
    const unsubDone = (window.api as any).agent?.onBashDone?.(() => { void loadLog() })
    const unsubImport = window.api.onImportCompleted?.(() => { void loadLog() })
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      const dirPath = detail.dir ? `_wiki/${detail.dir}${detail.page}.md` : `_wiki/${detail.page}.md`
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', padding: '6px 12px', gap: 8, borderBottom: '1px solid var(--color-border)' }}>
        ...
        {lastRefresh && <span style={{ fontSize: 10, color: 'var(--color-text-tertiary, #8e8e93)' }}>{lastRefresh}</span>}
        <span style={{ fontSize: 11, color: 'var(--color-text-tertiary, #8e8e93)', marginLeft: 'auto' }}>{content ? content.split('\n').length + ' 行' : ''}</span>
      </div>
      ...
      {loading ? (
        ...
        <div ref={containerRef}
          style={{
            ...
            fontFamily: 'var(--font-sans), -apple-system, sans-serif', fontSize: 12,
