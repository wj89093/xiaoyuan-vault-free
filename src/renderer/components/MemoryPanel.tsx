...
export function MemoryPanel({ onClose }: MemoryPanelProps): JSX.Element {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      if (!vaultPath) { setConversations([]); setLoading(false); return }
  ...
  const renderCard = (conv: ConversationSummary, i: number) => (
    <div key={i} className="memory-card" style={{ cursor: 'default' }}>
      <div className="memory-card-header">
        <Clock size={12} />
        <span className="memory-card-time">{conv.date} {conv.time}</span>
    ...
    <div role="region" aria-label="记忆面板" aria-busy={loading}>
      <FloatingPanel title="记忆" icon={<BarChart3 size={15} />} onClose={onClose}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '6px 12px', gap: 8, borderBottom: '1px solid var(--color-border)' }}>
          ...
          >
            <RefreshCw size={11} className={loading ? 'animate-spin' : ''} /> {loading ? '加载中...' : '刷新'}
          </button>
        </div>
        {loading ? (
          ...
          <div className="memory-panel-empty" role="status" aria-live="polite" style={{ color: 'var(--color-red, #ef4444)' }}>
            刷新失败：{loadError}
          </div>
