...
interface ParsedLintReport {
  date: string
  health: string
  stats: { totalWikiFiles: number; orphanPages: number; deadLinks: number; stalePages: number; contradictions: number }
  orphanPages: string[]
...
export function LintPanel({ onClose, vaultPath }: LintPanelProps): JSX.Element {
  ...
  async function loadLintReport() {
    ...
    try {
      ...
      if (!exists) { setReport(null); setLoading(false); return }
  ...
  useEffect(() => {
    const timer = setTimeout(() => { void loadLintReport() }, 0)
    return () => clearTimeout(timer)
  }, [vaultPath])

  const totalIssues = report ? report.stats.orphanPages + report.stats.deadLinks + report.stats.stalePages : 0
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <button
          onClick={() => { setRefreshing(true); void loadLintReport().finally(() => setRefreshing(false)) }}
          disabled={refreshing}
          style={{
            display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '4px 10px',
            borderRadius: 6, border: '1px solid var(--color-border)',
        ...
        <span style={{
          ...
          {runningCheck ? '检查中...' : loading ? '加载中...' : report ? (totalIssues > 0 ? `${totalIssues} 个问题` : '状态良好') : '暂无报告'}
        </span>
      </div>

      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading || runningCheck ? (
          <div style={{ color: 'var(--color-text-tertiary)', fontSize: 13, textAlign: 'center', padding: 32 }}>
            {runningCheck ? '检查中...' : '加载中...'}
          </div>
        ) : healthCheckError ? (
          <div style={{ color: 'var(--color-red)', fontSize: 13, textAlign: 'center', padding: 32 }}>
            检查失败，请重试
          </div>
        ) : loadError ? (
          <div style={{ color: 'var(--color-red)', fontSize: 13, textAlign: 'center', padding: 32 }}>
          ...
          <div style={{ textAlign: 'center', padding: 32, color: 'var(--color-green)', fontSize: 13 }}>
            ✓ 知识库状态良好
          </div>
        ) : (
          <>
            ...
            {report.stats.deadLinks > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{
                  ...
                  color: 'var(--color-orange)', padding: '8px 12px 6px',
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') setExpandedCats(c => ({ ...c, deadLinks: !c['deadLinks'] })) }}
                >
                ...
                {report.deadLinks.map((d, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{ color: 'var(--color-text-primary)', fontWeight: 500, marginBottom: 2 }}>
            ...
            {report.stats.orphanPages > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{
                  fontSize: 12, fontWeight: 600,
                  color: 'var(--color-text-tertiary)', padding: '8px 12px 6px',
                  cursor: 'pointer', userSelect: 'none',
                }}
                  tabIndex={0}
                  role="button"
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') setExpandedCats(c => ({ ...c, orphanPages: !c['orphanPages'] })) }}
                >
            ...
            {report.stats.stalePages > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{
                  fontSize: 12, fontWeight: 600,
                  color: 'var(--color-gray)', padding: '8px 12px 6px',
                  cursor: 'pointer', userSelect: 'none',
                }}
                  tabIndex={0}
                  role="button"
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') setExpandedCats(c => ({ ...c, stalePages: !c['stalePages'] })) }}
                >
