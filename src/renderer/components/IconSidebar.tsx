...
import { Search, Network, ClipboardCheck, BarChart3, Settings, FolderCog, FileText, FileOutput, StickyNote } from 'lucide-react'
...
export function IconSidebar({ activeView, onViewChange, onSearchFocus, onBriefingClick, onSchemaClick, onOpenTrash, onOpenOutput, onIndexClick }: IconSidebarProps): JSX.Element {
  ...
  const handleClick = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (id === 'search') { await onSearchFocus?.(); }
      else if (id === 'review') { await onBriefingClick?.(); }
      else if (id === 'lint') { await onViewChange(id); }
      else if (id === 'output') { await onOpenOutput?.(); }
      else if (id === 'schema') { await onSchemaClick?.(); }
      else if (id === 'log') { await onViewChange('log'); }
      else if (id === 'index') { await onIndexClick?.(); }
      else if (id === 'trash') { await onOpenTrash?.() }
      else { await onViewChange(id) }
  ...
  }, [activeView, onViewChange, onSearchFocus, onBriefingClick, onSchemaClick, onOpenTrash, onOpenOutput, onIndexClick])
  ...
  return (
    ...
    >
      ...
      {/* Settings */}
      <button
        ref={el => { buttonRefs.current[NAV_ITEMS.length] = el }}
        role="menuitem"
        tabIndex={focusedIndex === NAV_ITEMS.length ? 0 : -1}
        className={'icon-nav-item' + (activeView === 'settings' || clicked === 'settings' ? ' active' : '')}
        onClick={() => handleClick('settings')}
        onKeyDown={(e) => handleKeyDown(e, NAV_ITEMS.length)}
        title="设置"
        aria-pressed={activeView === 'settings' || clicked === 'settings'}
      >
        <Settings size={20} strokeWidth={activeView === 'settings' || clicked === 'settings' ? 2 : 1.5} />
      </button>
      {/* Trash */}
      <button
        ref={el => { buttonRefs.current[NAV_ITEMS.length + 1] = el }}
        role="menuitem"
        tabIndex={focusedIndex === NAV_ITEMS.length + 1 ? 0 : -1}
        className={'icon-nav-item' + (activeView === 'trash' || clicked === 'trash' ? ' active' : '')}
        onClick={() => handleClick('trash')}
        onKeyDown={(e) => handleKeyDown(e, NAV_ITEMS.length + 1)}
        title="回收站"
        aria-pressed={activeView === 'trash' || clicked === 'trash'}
      >
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={activeView === 'trash' || clicked === 'trash' ? 2 : 1.5} strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
      </button>
    </div>
  )
}
